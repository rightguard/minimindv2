...
class PretrainDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_length=512):
        ...
        # 使用huggingface的dataset的惰性加载，避免一次性读入大文件
        self.samples = load_dataset("json", data_files=data_path)["train"]
    ...
    def __getitem__(self, index):
        ...
        tokens = self.tokenizer(
            ...
            max_length=self.max_length - 2,  # eos和bos占位
        ...
        # 需要自行编写lables，防止pad参与loss计算
        labels = input_ids.clone()
        labels[labels == self.tokenizer.pad_token_id] = -100

        # 需要编写attention_mask，告诉模型哪些位置是有效的，哪些位置是pad
        attention_mask = (input_ids != self.tokenizer.pad_token_id).long()
        # 我们要输出的实input_ids, attention_mask, labels
        return {
            "input_ids": input_ids,
