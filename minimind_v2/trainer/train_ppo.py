...
def calculate_rewards(prompts, responses, reward_model, reward_tokenizer):
    def reasoning_model_reward(rewards):
        ...
        # re.S 是 Python 正则表达式模块中的一个标志位（Flag），它是 re.DOTALL 的缩写。
        # 功能： 在默认情况下，正则表达式中的点号 . 可以匹配除换行符 \n 以外的任何字符。
        # 为什么在这里必须用： 推理模型的回答（如 <think>...</think> 内部）通常包含多行内容。
        # 使用 re.S 后，. 就能匹配包括换行符在内的所有字符，从而能够抓取跨越多行的完整文本块。
    ...
    with torch.no_grad():
        ...
        for prompt, response in zip(prompts, responses):
            ...
            score = reward_model.get_score(reward_tokenizer, tmp_chat)  # ！修正：原get_reward(tmp_chat, reward_tokenizer)方法名和参数顺序错误

            # scale 在这里起到了 Reward Clipping（奖励裁剪） 的作用。
            # 防止数值不稳定： 奖励模型（Reward Model）输出的原始分数（Raw Score）有时会出现极端值。
            # 如果奖励值太大或太小，会导致强化学习（如 PPO）过程中的梯度爆炸或训练不稳定。
            # 归一化控制： 将分数强制限制在 [-3.0, 3.0] 之间，确保模型收敛更平滑。
            # 这是一种在工业界微调大模型时非常常见的工程实践。
            scale = 3.0
            score = max(min(score, scale), -scale)

            # args.reasoning 是一个开关参数，决定了是否开启“推理模型”特有的奖励机制。
            # args.reasoning == 1： 代表开启推理模式。此时代码会额外检查回答是否包含 <think> 和 <answer> 标签，并对格式的正确性给予奖励。
            # 这通常用于训练像 DeepSeek 那样能够主动思考的模型。
            # args.reasoning == 0： 代表普通模式。代码将忽略格式检查，只依赖基础的 Reward Model 给出的分数。
            if args.reasoning == 1:
                ...
                if answer_match:
                    ...
                    # 加权组合的原因及 score 与 answer_score 的区别
                    # 代码中通过 score * 0.4 + answer_score * 0.6 来计算最终分数，这反映了对过程与结果的双重考核：

                    # 各自代表的含义：
                    # score (全局分数)： 代表对 “Prompt + 完整回答”（包含整个思维过程和最终答案）的整体评估。
                    # 它衡量的是：推理过程逻辑是否通顺？是否绕了弯路？整体风格是否符合人类偏好？

                    # answer_score (答案分数)： 代表仅对 <answer> 标签内部内容 的评估。
                    # 它衡量的是：抛开中间思考过程，给出的最终结论是否正确、简洁、有用？

                    # 为什么要加权组合？
                    # 结果导向： 最终用户最关心的是答案。给 answer_score 0.6 的高权重，是为了强制模型必须输出高质量的结论。

                    # 约束思维过程： 如果只看 answer_score，模型可能会学会“作弊”（例如乱写一段思考过程，只要答案对了就行）。
                    # 给 score 0.4 的权重，是为了确保思维链（CoT）不仅存在，而且与上下文保持逻辑一致。

                    # 多目标优化： 这是一种平衡策略。0.6 和 0.4 是经验参数，旨在让模型既能“想得深（CoT质量）”，又能“答得对（结论质量）”。
