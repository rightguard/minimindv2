...
from trainer_utils import (  # 训练工具函数
    get_lr,
    Logger,
    is_main_process,
    lm_checkpoint,
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            current_loss = loss.item()  # 真实损失值，不再乘以 accumulation_steps
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            # torch.compile 包装后的模型通过 _orig_mod 访问原始模型
            raw_model = model
...
if __name__ == "__main__":
    ...
    if args.use_compile == 1:
        try:
            ...
            if "GPUTooOldForTriton" in str(type(e).__name__) or "Triton" in str(e):
                Logger("⚠️ GPU 不支持 torch.compile（CUDA Capability 太老），已自动关闭")
            else:
                raise
    ...
    if is_main_process():
        ...
        weight_path = f"{args.weight_dir}/{args.save_weight}_{lm_config.hidden_size}{moe_suffix}.pth"
        
        # 获取模型权重（处理 DDP 和 torch.compile 包装）
        raw_model = model
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            raw_model = model.module
        raw_model = getattr(raw_model, "_orig_mod", raw_model)
        ...
        Logger(f"训练完成！最终权重已保存到: {weight_path}")
        
        # 同时保存 config，方便 eval 阶段加载
        config_path = f"{args.weight_dir}/{args.save_weight}_{lm_config.hidden_size}{moe_suffix}_config.pth"
        config_data = {
