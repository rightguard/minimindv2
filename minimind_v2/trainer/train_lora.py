...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            # 📚 .item()方法
            # 将单元素张量转换为Python标量
            # loss 已不再除以 accumulation_steps，直接使用原始值
...
if __name__ == "__main__":
    ...
    # 为指定的层添加A和B矩阵
    # 先 unwrap DDP 和 torch.compile，避免 named_modules() 递归死循环
    base_model = model
    ...
    print(">>> 正在加载数据集，请稍候（首次运行可能需要1-3分钟）...")
    train_ds = SFTDataset(args.data_path, tokenizer, max_length=args.max_seq_len)
    print(f">>> 数据集加载完成，共 {len(train_ds)} 条样本")
    ...
    # 📚 梯度缩放器
    # GradScaler: 用于float16/bfloat16 训练的梯度缩放，防止梯度下溢
